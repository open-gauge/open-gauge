...
1. **PostgreSQL** (`postgres_data` volume) — everything structured: assets, calibrations,
   users, audit logs. The easiest path is the **Dangerous zone** at the bottom of
...
Whichever `docker compose` invocation you use for day-to-day operation, use the *same* one
consistently — Compose derives its project name (and therefore its volume names) from the
current directory unless overridden, and this repo's `docker-compose.yml` pins an explicit
`name: opengauge` specifically so this can't silently drift and point at a different, empty set
of volumes.
