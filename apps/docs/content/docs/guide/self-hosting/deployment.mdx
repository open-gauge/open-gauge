...
| `WARM_TECTONIC_CACHE` | `api` (build arg) | Pre-fetches Tectonic's LaTeX package bundle into the image at build time so certificate generation needs no network access at runtime. Keep `true` for any real/air-gapped deployment; set `false` only to work around a rate-limited bundle-mirror fetch during local builds. |
...
1. **PostgreSQL** (`infrastructure/docker/data/postgres`) — run `pg_dump`/`pg_restore` against the
   `db` service directly, or back up the directory itself with any regular file-level tool.
2. **MinIO** (`infrastructure/docker/data/minio`) — back up the directory directly, or use
   `mc mirror` against the MinIO S3 API.
