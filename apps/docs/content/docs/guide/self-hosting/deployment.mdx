...
directory, an explicit `-p` project override, or a legacy `docker-compose` v1 binary). If you're
upgrading from a version that used named volumes (`postgres_data`/`minio_data`), copy their
contents into the new paths once before redeploying:
