...
description: Running the full MAR stack with Docker Compose — services, ports, environment variables, backups, and migrations.
---

Docker Compose is MAR's primary, first-class deployment target. Every feature is expected to
work on it — from a Raspberry Pi or Intel N100 mini PC in a lab, to an internal company server.
Kubernetes support is optional and not required to run MAR.
...
| `web` | `apps/web/Dockerfile` | `3000` | The MAR user interface. |
...
Run this after pulling a new MAR version that includes schema changes, before restarting the
