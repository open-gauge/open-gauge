...
- **Asset "Interface" and "CAD" tabs.** An Electrical panel (connector image + pinout table with
  ...
  dialog) and a Mechanical panel (drawing + mounting-points table), each with independent
  Edit/Save/Cancel — plus a CAD tab for uploading and live-previewing STL/STEP/IGES/BREP files
  (STL via `three`, the rest triangulated in-browser by the `occt-import-js` OpenCascade WASM
  build). See [Asset registry → Interface tab](../sensors/asset-registry.mdx#interface-tab) and
