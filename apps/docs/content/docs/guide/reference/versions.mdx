...
bottom-left of the sidebar (`v4.1.0 · self-hosted`) and in the OpenAPI schema. This page mirrors
...
- **Calibration tab's saved-record view harmonized with the wizard's Step 3.** The asset detail
  ...
  - The model panel used a hand-rolled Equation/Coefficients tab toggle with plain-text
    ...
    coefficients. Both views now share the same `ModelPanel`/`Katex` component, so they can't
    drift apart again.
  - The statistics panel still showed a "Poly degree" row and a single combined "Valid range"
    row (reference domain only) — both dropped from the wizard's statistics panel in an earlier
...
- **Shared `Select` and `DatePicker` components, and app-wide numeric input standardization.**
  [UI.md](https://github.com/open-gauge/open-gauge/blob/main/UI.md) now defines consistent width
  tiers and behavior for every numeric input, dropdown, and date field in the app:
  - **Numeric inputs** — the chevron-button `NumberInput` component (previously only used in the
    Calibration Wizard's Step 3) is now used app-wide, sized to the value rather than the label
    (`w-16` to a `w-32` ceiling).
  - **Dropdowns** — a new shared `Select` component replaces the several near-duplicate per-page
    `<select>` wrappers. It stays a native `<select>` (full keyboard nav, mobile picker,
    screen-reader support) with a restyled chevron; the trigger caps at a width tier (`w-20` to a
    `w-64` ceiling) and ellipsizes long labels with a hover tooltip, while the native option
    list still auto-sizes to the widest option.
  - **Date pickers** — a new shared `DatePicker` component replaces the native
    `<input type="date">` with a themed popover calendar (month navigation, `og-accent` day
    markers, a month/year quick-jump view) — a date is always picked from the calendar, never
    typed, avoiding locale date-format ambiguity.
  - Applied across every screen that previously hand-rolled its own numeric input, `<select>`, or
    date input (admin, procedures, sites, assets, asset detail, calibration wizard, health tab,
    interface tab, organization detail).

## 4.3.0

### Added

- **Asset "Interface" and "CAD" tabs.** An Electrical panel (connector image + pinout table with
  multi-color wire swatches, signal name, description, and a click-to-place pin **Mapping**
  dialog) and a Mechanical panel (drawing + mounting-points table), both read-only until the
  page's own Edit button is used — plus a CAD tab for uploading and live-previewing
  STL/STEP/IGES/BREP files (STL via `three`, the rest triangulated in-browser by the
  `occt-import-js` OpenCascade WASM build), whose upload drop zone works the same way. See
  [Asset registry → Interface tab](../sensors/asset-registry.mdx#interface-tab) and
  [→ CAD tab](../sensors/asset-registry.mdx#cad-tab).

### Fixed

- Asset picture upload/removal (and the new pinout/mechanical-image uploads) no longer blank the
  header's calibration/location stats — they now reload the full profile after saving instead of
  trusting the upload endpoint's slimmer response shape directly.
...
- **Procedure export/import.** Admin and superadmin users can export procedures as ZIP bundles
  ...
  **+ New Procedure** for bulk actions, and **+ New Procedure → Import from file** handles a
  ...
  any non-viewer. See [Procedures → Import & export](../procedures/index.mdx#import--export).
...
- **Harmonized calibration detail view.** The Calibration tab's left-side record view now leads
  ...
  for the record's `data_entry_mode`. See
  [Viewing a calibration record](../calibration/overview.mdx#viewing-a-calibration-record).
...
- **Breaking: Frequency response is now a 4th calibration input mechanism, not a separate wizard
  ...
  [Frequency response](../calibration/frequency-response.mdx) guide page.
...
- **Calibration method for raw data.** Step 3 (Analysis) of a raw-data calibration offers a
  ...
  **Lookup Table** (no curve fit; the entered points *are* the model, linearly interpolated —
  ...
  as you type — see [Custom formula syntax](./custom-formulas.mdx).
...
- **Scrollbars now match the app's own colors instead of the browser default**, documented in
  `UI.md`'s new Scrollbars section.
- **Notification bell button now hovers the same way as the language/theme buttons.**
...
- **A user's profile picture on their `/users/{id}` detail page didn't open the preview modal
  on click** — it now uses the shared `ImageUploadField` (read-only mode), matching every
  other picture in the app.
- **Tooltips inside a collapsed panel's field rows (asset overview) were clipped.**
  `CollapsibleSection` no longer clips overflow to round its corners — the header button gets
  conditional rounding instead.
- **Tooltip "view documentation" links 404'd in demo mode** — `tooltip.tsx` now uses the
  locale-aware `Link` from `@/i18n/navigation` instead of plain `next/link`.
...
- **Assets are now navigated by their human-readable asset ID (e.g. `OG-00042`), not the
  ...
  instead of `id`. QR/sticker labels now encode the same `asset_id`-based URL. Every
  `/assets/{ref}` endpoint (and its nested resources) resolves `{ref}` as either the
  `asset_id` or the internal UUID, so old bookmarked or previously-printed UUID-based links
  keep working unchanged.

### Fixed

- **Demo mode 404'd when clicking an asset from the registry** — the registry's list/grid
  links were plain `<a href>` tags instead of next-intl's locale-aware `Link`, so the
  un-prefixed URL 404'd in the demo's static export (no middleware to add the locale prefix).
  Switched to `Link` from `@/i18n/navigation`, matching every other asset link in the app.
