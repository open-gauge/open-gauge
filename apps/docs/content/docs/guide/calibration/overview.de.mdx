...
1. **Allgemeine Informationen** — Kalibrierart (**intern**, gegen eine Referenz-Anlage, die Ihnen
   gehört, oder **extern**, von einem externen Labor), Kalibrierdatum, **Registriert von** (ein
   Dropdown der Organisationsmitglieder der Anlage, standardmäßig Sie selbst), ein optionaler
   Prüfer **Geprüft von**, Umgebungsbedingungen und (für interne Kalibrierungen) das verwendete Referenznormal und das
...
Jede Kalibrierung hat einen Status: **gültig**, **ausstehende Genehmigung**, **abgelehnt** oder
**entwertet**. Nur **gültige** Kalibrierungen zählen für die Kalibrierung des Kanals,
Fälligkeitsdatumsberechnungen und die Driftanalyse — die anderen drei werden ausgeschlossen,
genau wie es bei entwerteten Kalibrierungen schon immer der Fall war.

Wird in Schritt 1 ein Prüfer unter **Geprüft von** angegeben, wechselt die Kalibrierung in den
Status **ausstehende Genehmigung** statt gültig. Der Prüfer wird benachrichtigt (in der App und
per E-Mail, falls konfiguriert) und kann von derselben Zeile im Reiter Kalibrierung aus:

- **Genehmigen** — die Kalibrierung wird gültig, und die registrierende Person wird
  benachrichtigt.
- **Ablehnen**, mit optionaler Begründung — die Kalibrierung wird abgelehnt, die registrierende
  Person wird benachrichtigt, und dies ist endgültig: eine Korrektur bedeutet, eine neue
  Kalibrierung zu registrieren, nicht die alte zu bearbeiten.

Nur der zugewiesene Prüfer oder eine Admin-/Superadmin-Übersteuerung kann darüber entscheiden.

Bleibt **Geprüft von** leer, wird der Workflow vollständig übersprungen — die Kalibrierung wird
sofort als gültig erstellt, genau wie zuvor.

Der Schalter **„Gültige Kalibrierungen anzeigen"** (nur für Admins) steuert, ob ausstehende,
abgelehnte und entwertete Kalibrierungen überhaupt angezeigt werden — standardmäßig deaktiviert.

## Eine Kalibrierung entwerten

Die Kalibrierhistorie wird nie zerstört — es gibt keine Löschen-Schaltfläche, nur **Entwerten**,
beschränkt auf `admin`/`superadmin`. Entwerten ist von jedem Status aus erreichbar — gültig,
ausstehende Genehmigung oder abgelehnt — als jederzeit verfügbare Übersteuerung. Das Entwerten
eines Eintrags (über das Papierkorb-Symbol in seiner Zeile im Reiter Kalibrierung):
...
aus derselben Zeile **wiederherstellen**, was all das Obige rückgängig macht; die
Wiederherstellung setzt eine Kalibrierung stets auf den Status **gültig** zurück — sie versucht
nicht, eine zuvor abgelehnte Kalibrierung wieder auf abgelehnt oder ausstehende Genehmigung
zurückzusetzen. Siehe [Kalibrierungen](/docs/api/calibrations) in der API-Referenz für die genauen
Payloads von `DELETE /calibrations/{id}`, `POST /calibrations/{id}/restore`,
