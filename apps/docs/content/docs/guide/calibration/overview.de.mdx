...
2. **Eingabedaten** — beginnt mit einem Methoden-Selektor: **Referenz vs. Gemessen** (Standard —
   ...
   physikalischen Messwert, ohne Kurvenanpassung), **Modell (Übertragungsfunktion)** (ein vom
   Labor angegebenes Modell, ganz ohne Rohdatenpunkte — nicht verfügbar, wenn der Kalibrierzweck
   Nach Reparatur ist), oder **Frequenzgang** (ein Sweep aus Frequenz-/Referenz-/Messwertpunkten
   für Sensoren, die ein Signal im Frequenzbereich liefern — verfügbar für jeden Zweck,
   einschließlich Nach Reparatur — siehe [Frequenzgang](./frequency-response.mdx)). Siehe
...
flowchart TD
    ...
    FreqResp --> FreqEntry["Frequenz-/Referenz-/Messwert-Sweep<br/>eingeben (+ optionaler Phasenversatz)"]
    FreqRespAfter --> FreqEntry
    FreqEntry --> FreqCalc["Schritt 3: Referenzfrequenz wählen —<br/>Empfindlichkeit = gemessen/Referenz<br/>bei dieser Frequenz"]
    FreqCalc --> FreqOut(["Ergebnis: Empfindlichkeitswert,<br/>Empfindlichkeits-/Phasen-vs.-Frequenz-<br/>Diagramme — kein Unsicherheitsbudget,<br/>keine Konformität"])
...
Kurvengleichung (es gibt keine bekannte Übertragungsfunktion, die angepasst werden könnte), die
Wie-vorgefunden-Seite von Nach Reparatur fließt nie in das Fälligkeitsdatum oder den Reiter Zustand
ein, unabhängig davon, von welcher Basismethode aus gestartet wurde, und Frequenzgang hat weder
Unsicherheitsbudget noch Konformitätsprüfung — sein Ergebnis ist ein Empfindlichkeitsverhältnis,
keine Bestanden/Nicht-bestanden-Aussage. Siehe
[Alternative Dateneingabemodi für Kalibrierungen](./coefficients-only.mdx) für die vollständige
...
## Einen Kalibrierdatensatz ansehen [#viewing-a-calibration-record]

Die Auswahl einer Kalibrierung in der Verlaufsliste des Kalibrier-Tabs zeigt links den
vollständigen Datensatz — vereinheitlicht über alle `data_entry_mode`-Werte hinweg (Referenz vs.
Gemessen, Referenz vs. Angezeigt, Modell, Wie-vorgefunden/Wie-belassen und Frequenzgang zeigen alle
dieselbe Art von Panels, nur mit dem Modell/den Statistiken/den Diagrammen, die der jeweilige Modus
tatsächlich hat). Oben deckt ein einziges Metadaten-Panel jedes in Schritt 1 eingegebene Feld des
Datensatzes ab: Kalibrierdatum, Fälligkeitsdatum, wer es registriert hat und (falls zugewiesen) wer
es geprüft hat, Kalibrierart und -zweck, Kalibrierintervall, das Kalibrierlabor (aufgelöst zum
Hersteller, zur externen Organisation oder zum internen Standort, je nach Kalibrierart),
Zertifikatsnummer, internes Verfahren und Referenzanlage (Kalibrierungen im internen Labor),
Reparaturdatum/-beschreibung (Kalibrierungen Nach Reparatur), Umgebungsbedingungen und Notizen —
die Aktionen Genehmigen/Ablehnen und die Schaltfläche zum Herunterladen des Zertifikats befinden
sich in der Kopfzeile dieses Panels. Darunter folgen das Modell-/Gleichungspanel (wenn der Modus
eines hat), das Statistik-/Unsicherheits-/Konformitätspanel mit seinen Diagrammen (wenn der Modus
sie hat) und jedes modusspezifische Diagnosepanel (die Wie-vorgefunden-Zusammenfassung; das
Empfindlichkeitspanel des Frequenzgangs) — genau wie in Schritt 3 des Assistenten beim Erstellen
des Datensatzes.

Jede Zeile der Verlaufsliste ist außerdem mit ihrem **Kalibrierzweck** markiert, farbig nach Zweck
— **Erstkalibrierung** (blau), **Routine** (grün), **Nach Reparatur** (orange), **Verifizierung**
(lila) — sodass der Zweck, der später in die Beitragslogik des Zustand-Tabs einfließt, auf einen
Blick sichtbar ist.
