...
1. **Allgemeine Informationen** — Kalibrierart und -zweck (siehe unten), Kalibrierdatum,
   **Erfasst von** (eine Dropdown-Liste der Organisationsmitglieder der Anlage, standardmäßig Sie
   ...
   Dieser Schritt hat auch ein Kontrollkästchen **"Nur Koeffizienten (keine Rohdaten)"** für extern
   ausgestellte Zertifikate, die direkt ein Polynom angeben — siehe
...
übersprungen werden mit **"Nur Koeffizienten (keine Rohdaten)"**, wenn das Zertifikat direkt ein
Polynom angibt.
