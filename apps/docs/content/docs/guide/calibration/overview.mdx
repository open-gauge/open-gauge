...
2. **Input data** — starts with a method picker: **Reference vs Measured** (the default —
   ...
   **Reference vs Indicated** (reference vs. an already-converted physical-quantity reading, no
   curve fit), or **Model (transfer function)** (a lab-stated model, no raw data points at all —
   not offered when Calibration purpose is After Repair). See
...
and After Repair's as-found side never feeds the due date or Health tab, no matter which base
method it started from. See [Alternative calibration data-entry modes](./coefficients-only.mdx)
for the full field-by-field breakdown of every branch above.
