...
Each calibration's **version** (`v1`, `v2`, …) is its chronological position — by
`calibration_date`, not by when it was entered — among that asset's (or asset channel's)
calibration history. Versions are recomputed on every new save, so backfilling an older
calibration you forgot to enter earlier slots it into the correct position and shifts later
records up, rather than always appending at the end. A calibration's version is otherwise
stable: voiding a record (below) never renumbers anything.
