...
1. **General info** — calibration type and purpose (below), calibration date, **Registered by**
   ...
   **"Coefficients only (no raw data)"** checkbox for externally-issued certificates that state a
   polynomial directly — see [Coefficients-only calibrations](./coefficients-only.mdx).
...
For OEM, External Accredited Lab, and Customer's Asset, raw data entry can be skipped entirely
with **"Coefficients only (no raw data)"** when the certificate states a polynomial directly.
