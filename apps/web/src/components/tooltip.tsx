...
export function Tooltip({
  ...
  content: string;
  /** Path into the MAR Knowledge Center (e.g. from CHAN_DOCS_LINKS). When set, the trigger becomes a link that opens the full explanation in a new tab. */
  docsHref?: string;
  children: React.ReactNode;
}) {
  const trigger = docsHref ? (
    <a
      href={docsUrl(docsHref)}
      target="_blank"
